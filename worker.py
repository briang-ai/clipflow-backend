...
# --- Load env ---
load_dotenv()
print("WORKER VERSION: reel_compile_v1", flush=True)
    ...
    with engine.begin() as conn:
        conn.execute(
            sa.text(
                """
                INSERT INTO reels (
                    id, user_id, player_name, jersey_number,
                    game_date, bucket, s3_key, status, created_at
                )
                VALUES (
                    :id, :user_id, :player_name, :jersey_number,
                    :game_date, :bucket, :s3_key, 'processing', NOW()
                )
...
def process_compile_reel(job: dict):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Stitch with ffmpeg concat — no re-encode, fast and lossless
        ...
        # Update reel row with final S3 key and mark complete
        with engine.begin() as conn:
            conn.execute(
                sa.text(
                    "UPDATE reels SET s3_key = :key, status = 'complete' WHERE id = :id"
                ),
...
def main():
    ...
    while True:
        try:
            ...
            # Jobs are either a plain upload_id string (legacy process_upload)
            # or a JSON object with a "type" field (e.g. compile_reel)
            try:
                job = json.loads(raw)
